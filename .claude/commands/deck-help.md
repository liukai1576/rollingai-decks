...
2. 用人话输出一张「我能帮你做什么」的地图，按**用户想干的事**组织（不要
   ...
   **做一份新 deck** → `/new-deck`（引导选模板：rolling-deck 粒子地球风 /
   feishu-deck-h5 结构化渲染）
   **Keynote/PPT 转网页版** → `/keynote`
   **复用历史 deck 的页面拼新 deck** → `/splice` 或 admin 购物车
   **让 deck 动起来**（数字滚动/逐字浮现/卡片错落）→ `/animate`
   **deck 入库**（进管理平台：标签+缩略图+故事）→ `/ingest`
