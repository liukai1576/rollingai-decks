...
def render(deck_path: Path, output_dir: Path) -> None:
    ...
    for src in ASSETS_SRC.iterdir():
        ...
        if dst.exists():
            continue
        # v2 ships asset subdirs (photos/, slide-anim/) the template
        # references by relative path — copy them whole, not just top-level
        # files (shutil.copy2 on a dir would raise IsADirectoryError).
        if src.is_dir():
